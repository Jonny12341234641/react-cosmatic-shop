...
//             // const alternativeNames = altNames.split(",");
//             const formattedAltNames = typeof altNames === 'string' 
//             ? altNames.split(',') 
...
export default function UpdateProductPage() {
    ...
    const [productID, setProductID] = useState(location.state.productID);
    const [name, setName] = useState(location.state.name);
    // FIX: Join array to string for input display
    ...
    async function updateProduct(){
        ...
        try {
            // FIX: Only upload if NEW files are selected
            ...
            // FIX: Convert string back to array safely
            const formattedAltNames = altNames.split(",");

            const product = {
                productID : productID,
                name : name,
                altNames : formattedAltNames, // FIX: Correct variable name
                images : finalUrls,           // FIX: Uses correct images list
                description : description,
                price : price,
                labelledPrice : labelledPrice,
                category : category,
            ...
            <div className="w-full max-w-3xl bg-white/80 backdrop-blur-sm shadow-2xl rounded-2xl border-t-4 border-accent overflow-hidden">
                ...
                <div className="p-8 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="col-span-1">
                            <label className={labelStyle}>Product ID</label>
                            <input disabled className={inputStyle} value={productID} onChange={(e) => setProductID(e.target.value)} />
                        </div>
                        <div className="col-span-1 md:col-span-2">
                            <label className={labelStyle}>Product Name</label>
                    ...
                    <div>
                        <label className={labelStyle}>Alternative Names (Keywords)</label>
                        <input className={inputStyle} value={altNames} onChange={(e) => setAltNames(e.target.value)} />
                    </div>
                    <div>
                        <label className={labelStyle}>Description</label>
                        <textarea className={`${inputStyle} h-32 resize-none`} value={description} onChange={(e) => setDescription(e.target.value)} />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className={labelStyle}>Selling Price</label>
                            <input type="number" className={inputStyle} value={price} onChange={(e) => setPrice(e.target.value)} />
                        </div>
                        <div>
                            <label className={labelStyle}>Labelled Price</label>
                            <input type="number" className={inputStyle} value={labelledPrice} onChange={(e) => setLabelledPrice(e.target.value)} />
                        </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
